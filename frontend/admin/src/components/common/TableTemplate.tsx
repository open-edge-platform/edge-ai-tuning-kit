...
// Helper function to safely render data and prevent XSS
function sanitizeValue(value: unknown): React.ReactNode {
  if (value === null || value === undefined) {
    return '';
  }

  // If the value is a React element, return it as is (already safe)
  if (isValidElement(value)) {
    return value;
  }

  // For objects, arrays, or complex types, convert to JSON string
  if (typeof value === 'object' && value !== null) {
    return JSON.stringify(value);
  }

  // Simple values (strings, numbers, booleans) can be rendered safely
  return String(value);
}
  ...
  return (
    ...
    >
      ...
      {headers.map((header, idx) => {
        return <TableCell key={`Cell_${index}_${idx}`}>{sanitizeValue(data[header.id])}</TableCell>;
      })}
      {enableActions ? <TableCell align="center">{sanitizeValue(data.actions)}</TableCell> : null}
    </TableRow>
  );
}
