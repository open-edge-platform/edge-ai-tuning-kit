...
import { Task, TrainHistory, EvalHistory } from "@/hooks/llm-finetuning-toolkit/use-tasks";
...
export function TaskMetricsTab({ task }: { task: Task }) {
  ...
  const formatNumber = (value: number | string | (string | number)[] | null | undefined, defaultValue = "N/A", decimals = 4) => {
  ...
  const LearningRateTooltip = ({ active, payload, label }: TooltipProps<number, string>) => {
  ...
  return (
    <div className="space-y-4">
      {/* Training Loss - One card per row */}
      <Card className="p-4">
      ...
      {/* Evaluation Loss - One card per row */}
      <Card className="p-4">
      ...
      {/* Learning Rate Chart - One card per row */}
      <Card className="p-4">
        <h5 className="text-sm font-medium mb-2">Learning Rate Schedule</h5>
        ...
        <div className="mt-2 text-xs text-gray-600">
          Current:{" "}
          {trainEntries.length > 0 && trainEntries[trainEntries.length - 1]?.lr
            ? trainEntries[trainEntries.length - 1].lr.toExponential(4)
            : "N/A"}
        </div>
      </Card>

      {/* Memory Usage - Peak Memory Allocation - One card per row */}
      <Card className="p-4">
        <h5 className="text-sm font-medium mb-2">
          Peak Memory Allocation (GB)
        </h5>
        {trainEntries.length > 0 &&
        trainEntries.some((entry) => entry.peak_memory_alloc !== undefined) ? (
          <div className="h-60 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
      ...
      {/* Memory Usage - Peak Memory Reserved - One card per row */}
      <Card className="p-4">
        <h5 className="text-sm font-medium mb-2">Peak Memory Reserved (GB)</h5>
        {trainEntries.length > 0 &&
        trainEntries.some(
        ...
        <div className="mt-2 text-xs text-gray-600">
          ...
          trainEntries[trainEntries.length - 1]?.peak_memory_reserved
            ? `${(
                trainEntries[trainEntries.length - 1].peak_memory_reserved / 1e9
              ).toFixed(2)} GB`
            : "N/A"}
        </div>
      </Card>

      {/* Performance - Tokens per Second - One card per row */}
      <Card className="p-4">
      ...
      {/* Accuracy - Mean Train Accuracy - One card per row */}
      <Card className="p-4">
        ...
        <div className="mt-2 text-xs text-gray-600">
          ...
          trainEntries[trainEntries.length - 1]?.mean_token_accuracy !==
            undefined
            ? `${(
                trainEntries[trainEntries.length - 1].mean_token_accuracy * 100
              ).toFixed(2)}%`
            : "N/A"}
        </div>
      </Card>

      {/* Accuracy - Mean Evaluation Accuracy - One card per row */}
      <Card className="p-4">
        <h5 className="text-sm font-medium mb-2">
